...
*/
  ...
  double elapsed = (del_sec) + ((tv_e.tv_usec - tv_s.tv_usec) / 1000000.0);
  printf("-Elapsed Time-\n%f sec\n%d msec\n",elapsed,del_msec);
  //print_lst(lst,n);
  //float numba = 0.345f;
  //printf("%f\n", numba);
...
void init_float_lst(float *f, int n){
  int i;
  float x,m;
  m = (float)n;
  //  int seed = time(0) % 100;	/* seconds since 1/1/1970 */
  //  printf ("seed=%d\n", seed);
  srand(123412151);			/* SEED */
  for (i=0; i<n; i++) {
    //x = (float)rand();
    x = ((float)rand()/(float)(RAND_MAX)) * m;
    printf("%.3e ", x);
    *(f + i) = x;
  }
  printf("\n");
